  ...
  return (
    <section id="contact" style={{ padding: '10rem 6vw 8rem', background: 'rgba(255,255,255,0.015)', position: 'relative' }}>
          ...
          <TiltCard intensity={5} style={{ borderRadius: '8px', border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)', padding: '2.5rem' }}>
            ...
            {status === 'success' ? (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
                ...
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', border: '1px solid rgba(74,222,128,0.4)', background: 'rgba(74,222,128,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                </div>
                <p style={{ fontFamily: FONT_SERIF, fontSize: '1.3rem', fontWeight: 800, color: '#fafaf8', margin: 0 }}>Message sent.</p>
                <p style={{ fontFamily: FONT_SANS, fontSize: '0.85rem', color: 'rgba(255,255,255,0.45)', margin: 0 }}>Thanks for reaching out — I'll get back to you soon.</p>
                <motion.button onClick={() => setStatus('idle')} whileHover={{ background: 'rgba(255,255,255,0.08)' }}
                  style={{ marginTop: '0.5rem', fontFamily: FONT_MONO, fontSize: '0.6rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', background: 'rgba(0,0,0,0)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '4px', padding: '8px 16px', cursor: 'pointer' }}>
                  Send another
                </motion.button>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                ...
                {status === 'error' && (
                  <p style={{ fontFamily: FONT_MONO, fontSize: '0.62rem', letterSpacing: '0.1em', color: '#f87171', margin: 0 }}>
                    Something went wrong. Please try again or email directly.
                  </p>
                )}

                <motion.button type="submit" disabled={status === 'sending'} whileHover={{ background: '#fff' }} whileTap={{ scale: 0.98 }}
                  style={{ fontFamily: FONT_SANS, fontWeight: 600, fontSize: '0.85rem', letterSpacing: '0.05em', background: '#e8e0d0', color: '#0a0a0a', border: 'none', borderRadius: '4px', padding: '14px 24px', cursor: status === 'sending' ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'background 0.2s', opacity: status === 'sending' ? 0.6 : 1 }}>
                  {status === 'sending' ? 'Sending...' : 'Send message'} {status !== 'sending' && <Send size={14} />}
                </motion.button>
              </form>
            )}
          </TiltCard>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8rem', paddingTop: '2rem', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        <span style={{ fontFamily: FONT_MONO, fontSize: '0.6rem', letterSpacing: '0.12em', color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase' }}>© 2026 Ashwin Gupta — Personal Portfolio</span>
